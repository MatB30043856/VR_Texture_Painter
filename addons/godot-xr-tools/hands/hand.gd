...
# Used to track changes in grip buttons for scaling model
var other_hand
var grip_pressed = false
var grip_state_changed = false
static var init_dist_between_hands = 0.0
static var dist_between_hands = 0.0

# Used for tracking if user is drawing
var trigger_pressed = false
var trigger_state_changed = false
@onready var hand_ray = get_child(0)

# Used for dispatching shaders
var render_device
var shader
...
func _ready() -> void:
	...
	# Figure out which hand this is and get access to the other one
	if (get_name() == "LeftHand"):
...
func _process(_delta: float) -> void:
	...
	if _controller:
		...
		# Check if grip button was pressed with state change detection
		
		if (grip > 0.05) :
			if (grip_pressed == false):
				grip_state_changed = true
			else :
				grip_state_changed = false
			grip_pressed = true
		else : 
			if (grip_pressed == true):
				grip_state_changed = true
			else :
				grip_state_changed = false
			grip_pressed = false
		...
		if (trigger > 0.05) :
			if (trigger_pressed == false):
				trigger_state_changed = true
			else :
				trigger_state_changed = false
	...
	# Scale the model if both grip buttons are pressed
	if (grip_pressed && other_hand.grip_pressed):
		if grip_state_changed :
			init_dist_between_hands = global_position.distance_to(other_hand.global_position)
			print("Inital Distance Between Hands: %s" % init_dist_between_hands)
		dist_between_hands = global_position.distance_to(other_hand.global_position)
		print("Distance between hands: %s" % dist_between_hands)
		var delta_dist_between_hands = dist_between_hands - init_dist_between_hands
		%Loaded_Model.scale += Vector3(delta_dist_between_hands * 0.001, delta_dist_between_hands * 0.001, delta_dist_between_hands * 0.001)
		print("Set Model Scale to %s" % %Loaded_Model.scale)
	
	# Get the intersection with the model
